...
def test_media_errors_say_what_was_measured_and_what_is_allowed():
    """报错必须直接告诉用户"实际多少、要求多少"，不能只丢一个错误码。

    真事：用户传了一段 95 秒的音乐，ComfyUI 报错面板里只有 `AUDIO_DURATION_INVALID`——
    既看不出是时长还是格式的问题，也不知道该剪到几秒，而这些限制本来都是可操作的。
    码仍放在最前面，测试与日志照旧可以按码匹配。
    """
    with pytest.raises(media.MediaInspectionError) as too_long:
        media._validate("audio", "audio/mpeg", 1024, {"durationSeconds": 95.346938})
    assert str(too_long.value).startswith("AUDIO_DURATION_INVALID")
    assert "95.35 秒" in str(too_long.value) and "2–30 秒" in str(too_long.value)

    with pytest.raises(media.MediaInspectionError, match="AUDIO_FORMAT_INVALID") as wrong_format:
        media._validate("audio", "audio/flac", 1024, {"durationSeconds": 5})
    assert "audio/flac" in str(wrong_format.value)

    with pytest.raises(media.MediaInspectionError, match="VIDEO_FRAME_RATE_INVALID") as slow:
        media._validate("video", "video/mp4", 1024, {
            "width": 1280, "height": 720, "durationSeconds": 5, "frameRate": 12.0, "videoCodec": "h264",
        })
    assert "12，要求 24–60" in str(slow.value)

    # 合计超限也要说清是多少（这条是在集合校验里触发的，不是单文件）。
    with pytest.raises(media.MediaInspectionError, match="AUDIO_TOTAL_DURATION_INVALID") as total:
        media.validate_media_collection([
            {"descriptor": {"role": "reference_audio", "metadata": {"durationSeconds": 20}}},
            {"descriptor": {"role": "reference_audio", "metadata": {"durationSeconds": 20}}},
        ])
    assert "40.00 秒" in str(total.value)


def test_audio_duration_is_normalized_to_a_tenth_of_a_second(tmp_path, monkeypatch):
    """mp3 的时长靠帧计数，不同 ffprobe 版本对同一个文件给出不同的值，必须归一后再比对。

    实测同一个 10 秒 mp3：容器里的 ffprobe 5.1.9 报 10.03102、本机 8.0.1 报 10.0。预检元数据
    按内容摘要与服务端逐字段比对，不归一就会在正式提交时撞 PREFLIGHT_ACTUAL_CONTENT_MISMATCH
    （2026-09-16 首次提交纯音频任务时真实踩到）。音频时长不进入计费公式，粒度足够。
    """
    ...
    item = media.inspect_media(path, "reference_audio", "reference-audio-1")

    assert item["descriptor"]["metadata"] == {
        "kind": "audio", "durationSeconds": 10.0, "audioCodec": "mp3",
    }
