...
> 归档说明：本文仍使用旧 `mode=preview` Task、`production_verified` 单状态、先上传后 Preview 和旧节点流程，已被 [v2 重构期间使用说明](../runbooks/creative-user-guide.md)取代。本文只保留历史操作方式，不得用于当前新建任务。
>
> 最后更新：2026-09-11
...
> 本节对应的旧一键脚本流程同样属于历史操作，不再作为当前新建任务入口。
