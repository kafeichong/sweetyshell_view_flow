# ComfyUI Video Flow Client（历史说明，已停用）

> 归档日期：2026-09-15。本文保留 v1 节点、安装和交付说明原貌，仅供追溯；其中旧 Preview 会上传素材，旧 Production/OneClick 不符合当前 v2 预检与确认合同，不得作为当前安装或使用指南。当前边界见 [创意同事使用说明](../runbooks/creative-user-guide.md)，实现状态只看 [PROJECT_STATUS](../PROJECT_STATUS.md)。
...
- `Seedance Product Video (One Click)`
- `Seedance Text to Video (Preview)`
- `Seedance First Frame to Video (Preview)`
- `Seedance First and Last Frame to Video (Preview)`
- `Seedance Multi Reference (Preview)`
- `Wait Video Flow Task`
- `Load Video Flow Result`
- `Preview Local Video`

`Seedance Reference Image to Video` 是当前唯一可进入 Production 的节点，正式接口支持 Registry 声明的 4–30 秒、标准画幅和 480p/720p/1080p 参数；服务端会按官方 Token 估算公式预占额度，仍受 actor 白名单和实际额度限制。`Seedance Text to Video (Preview)`、首帧、首尾帧和多参考模板只会生成 Preview 报告，不会调用 Provider；它们待单独完成账号级真实验收后才可能开放 Production。多参考模板当前只提供最多三张本地参考图，未开放视频/音频输入。

Seedance 2.5 的官方计费是“Token 单价 × Token 用量”：预估用量为 `(输入视频时长 + 输出视频时长) × 输出宽 × 输出高 × 输出 FPS ÷ 1024`，参考图没有输入视频时长；最终结算以 Provider 返回的 `usage.completion_tokens` 为准。当前 `seedance-2-5-official-v1` 使用官方无输入视频单价 480p/720p 为 70 元/百万 Token、1080p 为 77 元/百万 Token。

四类 Preview 模板均默认 `preview` 且关闭确认开关。Preview 只发送 Prompt、generation 和素材描述/哈希，不上传素材、不创建付费任务。正式模板使用 `Download` 输出的本地视频路径连接 `Preview Local Video`，ComfyUI 0.35.x 通过 MP4 的 `ui.images + animated` 兼容格式显示播放器。
