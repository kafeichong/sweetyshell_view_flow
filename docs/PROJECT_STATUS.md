...
3. **上传入库链路（免费，验证完已删除测试素材与组）**：把我方对象存储的签名地址交给 `CreateAsset`，**方舟在创建那一刻即拉取**——`Processing → Active` 只用了 **6.1 秒**，回传地址落在方舟自己的桶上。这条就是「以后从客户端上传素材入库」要走的路，**官方也确实支持**（权益表里虚拟人像素材入库写着「支持 Assets API 集成；支持控制台使用」）。**推论**：签名用默认短有效期即可，**不要**因为"CreateAsset 是异步的"去签长 TTL——实测没有这个必要，而长 TTL 等于给一张人脸图开长期可读的口子。素材库的成本模型是「订阅 + 素材/素材组数量配额 + QPM 限流」，**建/查/删素材都不按次计费**。
