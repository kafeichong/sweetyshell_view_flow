...
**⚠️ 执行方式要改（用户 2026-09-16 指示）**：上面这两条验收都是**命令行工具**跑的（`seedance_production_acceptance.py`），只证明了接口与计费链路通，**没有**证明界面可用。用户随后定规矩：**凡有花销的验证一律由他在 ComfyUI 客户端里手动 Queue** —— 一是测 UI 可用性，二是测真实客户端链路。runbook §4 已按此改写，我方只做准备（素材、免费预检报价）与只读取证、入账。
