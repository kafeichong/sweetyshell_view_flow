...
- 2026-09-14：Worker 新增纯 `seedance_execution_policy.py`，将已冻结的 reference-image、首尾帧、全模态参考、视频编辑和视频延长意图编译为 Ark `content` payload，并在提交前复验角色、顺序、数量、`adaptive` / `duration=-1` 与 `reference/edit/extend` 字段。当前仅有离线单测，尚未接入 `executor.py` 或 `SeedanceAdapter`，所以没有改变 Provider 请求。验证：`cd packages/worker && venv/bin/python -m pytest -q`（196 passed / 26 skipped / 12 deselected）。
