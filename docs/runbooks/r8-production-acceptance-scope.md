...
**付费那一步一律走 ComfyUI 客户端（§4.2，人工操作）；本节的命令行只用于不付费的准备与取证步骤**（2026-09-16 用户定的规矩）。理由是脚本绕开了界面：它验证不了 UI 可用性（下拉、上传按钮、提示与报错文案、Queue 流程），也验证不了真实客户端→后端→Provider 的完整握手——而这两样才是创意同事实际要用的东西。所以我方（AI/工具）只做到：准备素材、`upload` 拿 assetId、`preflight` 拿报价、把"在哪个模板点什么"写清楚、跑完用只读命令取证并入账。
...
**执行方式备注**：本节的音频参考首轮验收是用 §4.1 的命令行工具跑的（**不是** §4.2 的 ComfyUI 人工路径），所以它证明的是"接口与计费链路通"，**没有**证明界面可用。用户随后定下规矩：付费步骤一律由他在 ComfyUI 里 Queue（见 §4），此后本节的验证要按那条走。

**UI 路径验收已补（2026-09-16）**：用户在 ComfyUI 客户端里手动 Queue 跑通同一条（任务 `2c9e4579-4b89-40e2-8ac6-a025e9d603a4`、Provider 任务 `cgt-20260916164552-29ii0`、槽 `slot-template-audio-reference-v1`），产物 1280×720 / h264 / 3,665,101 字节 / 121 帧，`verify` 12 项全过；预占 `7.623000` → 结算 **`7.623000`**，与脚本那次一致。**而且这次连客户端的交付确认都走完了（`clientDelivery=delivered`，脚本那两次都是 `pending`）**——这是第一条真正证明「界面可用 + 客户端链路正确」的验收，已作为第二条记录写进合同 `validation.records`。
...
**放行当场暴露并修复的一个缺陷（影响全部含音频的工作流）**：首次用真实 mp3 提交正式任务时被 `PREFLIGHT_ACTUAL_CONTENT_MISMATCH` 拒掉——**同一个 10 秒 mp3，容器里的 ffprobe 5.1.9 报 10.03102 秒、本机 8.0.1 报 10.0**（mp3 时长靠帧计数，版本间算法不同）。视频那条 9/15 靠"改读流时长"解决了，音频不行（流时长本身随版本变），因此两边都把**音频时长按 0.1 秒归一**后再比对；音频时长不进入计费公式，粒度足够。已提交 `9f42dd7` 并部署。**注意**：内容寻址的资产复用不会重新检查，按旧代码检查过的音频资产仍带未归一的时长，用同一文件重试会再次被拒——需要换一份内容不同的文件，或另做一次资产重检/数据修复。**2026-09-16 已按后者处理**：生产库里当时共 2 个音频资产，其中用户那个 mp3（`406b13dd…`）的 `durationSeconds` 是旧值 `10.03102`，已用一条 UPDATE 对齐成 `10.0`（只动这一个字段，字节、哈希、其它字段不碰），随后同一个文件在 ComfyUI 里重试通过了。

**遗留的系统性问题**：只要检查器算出的值还会随版本变化，"已复用资产带着旧检查结果"这件事就会再次发生（不止 mp3）。彻底解法是给资产记一个"由哪版检查器检查"的标记，并在检查器升级后把旧资产判为需要重检（可以仿照现有的 `packages/backend/src/tools/asset-hash-backfill.ts` 做一个重检工具）。**尚未实现**。

**视频编辑首轮真实验收（2026-09-16 已完成，同样是命令行路径）**：任务 `4ebcf666-c9ff-4c72-bbe0-76e2379cc771`、Provider 任务 `cgt-20260916163145-9am0c`，槽 `r8-video-edit-5`，输入一段 786×1180 / 5.041667 秒的 mp4（我们自己早前的产物），请求 `duration=-1 / ratio=adaptive / outputFormat=mov / 720p`。产物 **MOV / h264 / 786×1180（画幅跟随输入）/ 121 帧 / 4,310,592 字节**（探针 5.041667 秒），`verify` 12 项全过，SHA-256 `7d824475…`。预占 `9.335760` → 结算 **`9.167886`**（218,283 tokens × 42 元/百万），**预占偏高、方向安全**。

**上面第 2 条的实测结论（重要）**：编辑**同样没有生成类那多出的一帧**——结算 `218,283 = (输入 5.041667 + 输出 5.0) 秒 × 786×1180 × 24 / 1024` 向下截断，其中"输出 5.0 秒"正是 121 帧按官方"总帧数 ÷ 24 向下取整"的口径。至此**两类特殊任务（延长、编辑）各自只有一个样本、都不加帧，而生成类 8 条样本全部加帧**；仍不据这两条样本改预占公式（只会高估），已作为第二个样本记进合同 `pricing.estimate.reservationAdjustment`。
