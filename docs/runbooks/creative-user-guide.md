...
| `seedance-video-extend-preflight-v1.comfy.json` | 视频延长（接原视频往前/往后续写 N 秒） | **至少 1 段参考视频**（≤10 段、总时长 ≤30 秒） | 延长 5 秒 + 5 秒输入视频约 9.11 元；输入视频越长越贵 |

> **视频延长**的提示词写法与其他模板不同：官方写法是「**在 @视频1 的基础上（向后）续写 N 秒的视频**，接着描述续写部分的内容」，**不要写时间戳分镜**。画幅自动跟随输入视频，输出固定为 mov。
>
