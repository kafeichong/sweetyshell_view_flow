...
**官方计算器自身的公式（更直接的证据）**：`seedance2.5价格计算器`（同一批官方链接之一）把它的计算逻辑完整嵌在页面的 `window.formMetaContent` 里，原文为：

```text
估算 token 用量 = (输入视频时长 + 输出视频时长) × 长边 × 短边 × 帧率 / 1024

最低 token 用量 = 在最低 token 表中 FILTER(
                    输入时长 ≠ "" &&
                    分辨率 = 选定分辨率 &&
                    宽高比 CONTAIN 选定宽高比 &&
                    输出时长 = 选定输出时长 )

视频价格 = IF(输入是否包含视频 = "输入包含视频",
              单价 × MAX(估算token用量, 最低token用量) / 1000000,
              单价 × 估算token用量 / 1000000)
```

注意最后的 `MAX(估算 token 用量, 最低 token 用量)`——**"取估算值与最低值的较大者"这条规则出自官方计算器自己的公式**，与本实现一致，因此它不再只是由数据拟合出来的经验规律。

**`5/3` 系数的来源（已补齐）**：官方最低 token 表不是只存一个数值，它同时存了**输入时长**列。逐行读取该列后发现：

```text
官方表存的 输入时长 == ceil(2 × 输出时长 / 3)      140/140 行吻合，覆盖输出 4–30 秒全部取值
```

而 `ceil(2D/3) + D ≡ ceil(5D/3)`，所以"最低计费总秒数 = `ceil(输出时长 × 5/3)`"并不是我另起的一条规则，**它恰好等于官方表自己存的那条输入时长下限加上输出时长**。至此本实现的三部分——计费口径 `MAX(估算, 最低)`、token 估算公式、最低用量取值——均有官方依据。

**仍需注意的边界**：官方来源的输入时长**全部是整数秒**（表按整数列存、计算器是 2–30 的下拉），**没有覆盖非整数秒**；本实现按精确小数秒计算（验收时那段参考视频为 11.966667 秒），这一点无官方依据，属于"照公式连续外推"。实际账单以 Provider 返回的 `usage.completion_tokens` 为准，该差异只影响预占金额。
