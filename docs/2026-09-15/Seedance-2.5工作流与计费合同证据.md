...
**仍需注意的边界**：官方表是一张按时长/比例/长边/短边列出的**查找表**，`ceil(输出×5/3)` 这个系数没有出现在任何官方文字里——它是我对最低 token 表逐行复现的结果（官方表本身按"输出时长 + 该行的输入时长下限"存值）。另外官方计算器的输入时长是**整数秒下拉（2–30）**，**未覆盖非整数秒**；本实现按精确小数秒计算，这一点无官方依据。
