...
官方指向的明细表是 Lark 快查表（`bytedance.larkoffice.com/wiki/FXaYwxzJ5i5Zdik32ipcWzt7nxd`），**该地址需要登录、本机不可直接访问**。规则由官方价格示例与公开快查表数据反推得到：
...
**取证边界**：这是从官方价格示例与公开快查表数据反推并按全部可得数据点验证的结果，不是从 Lark 原表逐行抄录。合同的 `pricing.inputVideoMinimumTokens.evidence` 如实记录了来源与验证点数，未伪装成直接取证。
